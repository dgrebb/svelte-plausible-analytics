import {} from './PlausibleAnalytics.svelte';
/**
 ...
 */
export const handleEntry = function handleEntry(entry) {
    const typeName = Object.prototype.toString.call(entry).slice(8, -1);
    const warn = (type) => console.warn(`Warning: Passing ${type} to Plausible may result in error unless parsed as a string.`);
    switch (typeName) {
        case 'DOMTokenList':
        ...
        case 'Number':
            return true;
        default:
            console.warn(`Plausible Error: Custom property entry ${entry} is not a boolean, number, or string.`);
            return true;
    }
};
/**
 ...
 */
export const isCustomPropsLimit = function isCustomPropsLimit(props) {
    const limit = 30;
    if (typeof props === 'object' && Object.entries(props).length > limit) {
        console.warn(`Warning: Plausible only allows up to 30 custom properties per event. ${props.length} properties counted.`);
    }
    return true;
};
/**
 ...
 */
export const isCustomPropEntryLimit = function isCustomPropEntryLimit(limit, entry) {
    const asString = entry.toString();
    const warning = (length) => `Warning: Plausible limits custom property ${limit > 300 ? 'values' : 'names'} to ${limit} characters. ${entry} is ${length} characters.`;
